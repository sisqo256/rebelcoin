...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s RBL additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	if (nHeight == 1)
		nSubsidy = 573319.86 * COIN;
	else if (nHeight == 16000000)
		nSubsidy = 4.1487656 * COIN;
	else
		nSubsidy = 37.648 * COIN;
 ...
 *
 * Non-static (and re-declared) in rebelcoin/test/txvalidationcache_tests.cpp
 */
