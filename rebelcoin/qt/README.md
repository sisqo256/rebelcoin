...
The current precise version for Qt 5 is specified in [qt.mk](/deps/packages/qt.mk).
...
ClientModel is used by the main application `rebelcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/rebelcoin/rebelcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a rebelcoin:... link from a browser).
...
* `rebelcoinstrings.cpp`: automatically generated
* `rebelcoinunits.(h/cpp)`: BTC / mBTC / etc handling
* `callback.h`
...
* don't change `local/rebelcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
4. Enter "rebelcoin-qt" as project name, enter rebelcoin/qt as location
5. Leave the file selection as it is
6. Confirm the "summary page"
7. In the "Projects" tab select "Manage Kits..."
8. Select the default "Desktop" kit and select "Clang (x86 64bit in /usr/bin)" as compiler
9. Select LLDB as debugger (you might need to set the path to your installation)
10. Start debugging with Qt Creator (you might need to the executable to "rebelcoin-qt" under "Run", which is where you can also add command line arguments)
