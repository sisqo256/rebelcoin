 ...
 */
static const char* BITCOIN_PID_FILENAME = "rebelcoind.pid";
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (fs::exists(config_file_path)) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Rebelcoin (RBL) config file:\n"
                                    "rpcuser=username\n"
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if rebelcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if rebelcoin is started while in a temporary directory.\n",
