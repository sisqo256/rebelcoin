...
const char * const BITCOIN_CONF_FILENAME = "rebelcoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "rebelcoind -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //rebelcoin-tx using stdin
        std::string val;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Rebelcoin";
#else
    // Unix
