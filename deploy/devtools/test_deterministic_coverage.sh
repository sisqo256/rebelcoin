...
NON_DETERMINISTIC_TESTS=(
    "blockfilter_index_tests/blockfilter_index_initial_sync"  # rebelcoin/checkqueue.h: In CCheckQueue::Loop(): while (queue.empty()) { ... }
    ...
    "miner_tests/CreateNewBlock_validity"                     # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "scheduler_tests/manythreads"                             # scheduler.cpp: CScheduler::serviceQueue()
    "scheduler_tests/singlethreadedscheduler_ordered"         # scheduler.cpp: CScheduler::serviceQueue()
    "tx_validationcache_tests/checkinputs_test"               # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "tx_validationcache_tests/tx_mempool_block_doublespend"   # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "txindex_tests/txindex_initial_sync"                      # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "txvalidation_tests/tx_mempool_reject_coinbase"           # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "validation_block_tests/processnewblock_signals_ordering" # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/coin_mark_dirty_immature_credit"            # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/dummy_input_size_test"                      # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/importmulti_rescan"                         # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/importwallet_rescan"                        # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/ListCoins"                                  # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/scan_for_wallet_transactions"               # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/wallet_disableprivkeys"                     # rebelblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
)

TEST_BITCOIN_BINARY="rebelcoin/test/test_bitcoin"
...
get_file_suffix_count() {
    find rebelcoin/ -type f -name "*.$1" | wc -l
}
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    find rebelcoin/ -type f -name "*.gcda" -exec rm {} \;
    ...
    if ! gcovr --gcov-executable "${GCOV_EXECUTABLE}" -r rebelcoin/ > "${GCOVR_TEMPFILE}"; then
